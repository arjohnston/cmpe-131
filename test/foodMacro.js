...
const FoodMacro = require('../api/models/FoodMacro')
...
describe('Food Macro', () => {
  ...
  describe('/POST /api/foodMacro/create', () => {
    // Register a user
    ...
    it('Should return statusCode 200 when submitting an object with an object', done => {
      chai
        .request(app)
        .post('/api/foodMacro/create')
        .send({
  ...
  describe('/POST /api/foodMacro/getMacros', () => {
    it('Should return statusCode 401 when a token is not passed in', done => {
      chai
        .request(app)
        .post('/api/foodMacro/getMacros')
        .send({})
        .then(function (res) {
          expect(res).to.have.status(BAD_REQUEST)
    ...
    it('Should return all macros in an array', done => {
      chai
        .request(app)
        .post('/api/foodMacro/getMacros')
        .send({ token: token })
